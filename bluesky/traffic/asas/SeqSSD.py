...
def SSDVariables(asas, ntraf):
    ...
    asas.FRV          = [None] * ntraf
    asas.FRV_3      = [None] * ntraf #NEW
    asas.FRV_5      = [None] * ntraf #NEW
    asas.FRV_min    = [None] * ntraf #NEW
    asas.FRV_tla        = [None] * ntraf #NEW
    asas.FRV_dlos        = [None] * ntraf #NEW
    
    #Allowed Reachable Velocity regions
    asas.ARV          = [None] * ntraf
    asas.ARV_3          = [None] * ntraf #NEW
    asas.ARV_5          = [None] * ntraf #NEW
    ...
    asas.FRV_area_5 = np.zeros(ntraf, dtype=np.float32) #NEWWW
    asas.FRV_area_3 = np.zeros(ntraf, dtype=np.float32) #NEWWW
    asas.FRV_area_min = np.zeros(ntraf, dtype=np.float32) #NEWWW
    asas.FRV_area_tla = np.zeros(ntraf, dtype=np.float32) #NEWWW
    asas.FRV_area_dlos = np.zeros(ntraf, dtype=np.float32) #NEWWW
    
    
    asas.ARV_area     = np.zeros(ntraf, dtype=np.float32)
    asas.ARV_area_5 = np.zeros(ntraf, dtype=np.float32) #NEWWW
    asas.ARV_area_3 = np.zeros(ntraf, dtype=np.float32) #NEWWW
    asas.ARV_area_min = np.zeros(ntraf, dtype=np.float32) #NEWWW
    asas.ARV_area_tla = np.zeros(ntraf, dtype=np.float32) #NEWWW
    asas.ARV_area_dlos = np.zeros(ntraf, dtype=np.float32) #NEWWW
...
def constructSSD1(asas, traf):
    ...
    for i in range(ntraf):
        ...
        if asas.inconf[i] == True:
            ...
            pc_5.AddPaths(pyclipper.scale_to_clipper(circle_tup), pyclipper.PT_SUBJECT, True)
            pc_3.AddPaths(pyclipper.scale_to_clipper(circle_tup), pyclipper.PT_SUBJECT, True)
            ...
            min_dlos = min(dlos)
            ind_mindlos = [index for index, item in enumerate(dlos) if item <= min_dlos + 60 ] #the threshold is only to make sure computational errors don't mess this 
            ...
            for j in range(np.shape(i_other)[0]):
                ...
                if dist[ind[j]] > hsepm:
                    ...
                    pc_aux.AddPaths(pyclipper.scale_to_clipper(circle_tup), pyclipper.PT_SUBJECT, True)
                    ...
                    VO_5,_ = roundoff(tau, R_pz, dist_mod, VO_points[0,:], nd, n_t1, n_t2, vertexes, xyc)
                    pc_5.AddPath(pyclipper.scale_to_clipper(VO_5), pyclipper.PT_CLIP, True)
                    ...
                    VO_3,_ = roundoff(tau, R_pz, dist_mod, VO_points[0,:], nd, n_t1, n_t2, vertexes, xyc)                        
                    pc_3.AddPath(pyclipper.scale_to_clipper(VO_3), pyclipper.PT_CLIP, True)
            ...
            FRV_5 = pyclipper.scale_from_clipper(pc_5.Execute(pyclipper.CT_INTERSECTION, pyclipper.PFT_NONZERO, pyclipper.PFT_NONZERO))
            ARV_5 = pyclipper.scale_from_clipper(pc_5.Execute(pyclipper.CT_DIFFERENCE, pyclipper.PFT_NONZERO, pyclipper.PFT_NONZERO))
            
            #3mins layer (cuts @3mins)
            FRV_3 = pyclipper.scale_from_clipper(pc_3.Execute(pyclipper.CT_INTERSECTION, pyclipper.PFT_NONZERO, pyclipper.PFT_NONZERO))
            ARV_3 = pyclipper.scale_from_clipper(pc_3.Execute(pyclipper.CT_DIFFERENCE, pyclipper.PFT_NONZERO, pyclipper.PFT_NONZERO))
...
def calculate_resolution1(asas, traf):
    ...
    gseast = traf.gseast
    gsnorth = traf.gsnorth
    #asas.layers_dict = {0: 'No solution', 1: 'Full FRV layer', 2: 'FRV zones within tla', 3: 'Current conflicts within tla', 
                            #4: 'FRV zones at minimum time to LoS', 5: 'Current conflicts at minimum time to LoS',
                            #6: 'Current conflicts at minimum distance to LoS'}
                ...
                for no_layer in layers:
                    ...
                    if len(layer) >0:
                        ...
                        if sol_mod< vmax and sol_mod > vmin and turn_angle < turn_cons:                         
                            ...
                            #asas.reso_layer[i] = asas.layers_dict[j]
